...
[[18-5-early-binding-and-late-binding|18.5 - 早绑定和晚绑定]]

- 当编译器解析到函数的直接调用时，会进行[[Early-binding|早绑定]]。编译器或者链接器可以直接解析该函数调用。 [[Late-binding|晚绑定]] 则发生在通过函数指针进行调用时。在这种情况下，实际的函数调用只有在运行时才能确定。虚函数使用[[Late-binding|晚绑定]]和虚函数表来确定实际调用的函数。
