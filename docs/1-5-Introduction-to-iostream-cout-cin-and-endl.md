...
>[!TIP] 最佳实践
> 尽管，关于一个变量在用于接收用户输入（例如通过 `std::cin`）的数据之前，是否应该进行初始化这个问题有很多争论，毕竟用户的输入会覆盖初始化的值。与我们之前的建议类似，变量在使用前都必须要进行初始化，因此这里的最佳实践仍然是，变量要先进行初始化再用于接收用户输入。

我们会在 [[7.16 -- std::cin and handling invalid input]] 中讨论 `std::cin` 如何应对非法输入。

>[!INFO] 扩展阅读
> C++ 的输入输出库并没有提供一种无需按下回车就可以获取输入结果的方法。如果你希望实现该功能，你必须使用第三方库来完成。如果是开发控制台应用程序，我们推荐尝试 [pdcurses](https://pdcurses.org/) 这个库。另外，很多图形用户库界面库也提供了类似的功能。
...
-   `<<` 配合 `std::cout` 使用，同时也表明了数据的流动方向 (假设 `std::cout` 表示的是控制台，则输出的数据从变量流向控制台）。`std::cout << 4` 就表示将4发送到控制台。
- `>>` 配合 `std::cin` 使用，同时也表明了数据的流动方向 (假设 `std::cin` 表示的是键盘，则输入的数据是从键盘流向变量）。`std::cin >> x` 就表示从键盘获取的用户输入被存放到变量 x 中

更多关于运算符的详细内容，将在 [[1.9 字面量和运算符]] 中进行介绍。