...
相反，C++提供了解决该问题的另外一种途径：允许我们在程序中创建全新的自定义类型！这样的类型通常称为用户定义类型(尽管我们认为术语程序定义类型更好——我们将在本课后面讨论其区别)。C++有两类复合类型可以实现这一点：枚举类型(包括[[unscoped-enumerations|无作用域枚举类型]]和[[scoped-enumerations|限定作用域枚举]])和类类型(包括结构体、类和联合体)。
