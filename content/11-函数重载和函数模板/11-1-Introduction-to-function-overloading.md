...
> - 重载函数能够进行编译，必须满足两个条件：
>   1. 每个重载的函数都必须能够区分。我们会在 [[11-2-Function-overload-differentiation|11.2 - 重载函数的区分]] 中介绍它们是如何被区分的。
>   2. 每个被调用的重载函数在重载解析时必须能够定位到一个函数。我们会在[[11-3-Function-overload-resolution-and-ambiguous-matches|11.3 - 函数重载解析和不明确匹配]]中介绍如何进行重载函
...
1. 每个重载的函数都必须能够区分。我们会在 [[11-2-Function-overload-differentiation|11.2 - 重载函数的区分]] 中介绍它们是如何被区分的。
2. 每个被调用的重载函数在重载解析时必须能够定位到一个函数。我们会在[[11-3-Function-overload-resolution-and-ambiguous-matches|11.3 - 函数重载解析和不明确匹配]]中介绍如何进行重载函数的匹配。
