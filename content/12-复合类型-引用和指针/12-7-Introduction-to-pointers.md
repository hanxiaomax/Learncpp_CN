...
> [!info] "相关内容"
> 如果你还不熟悉左值引用，现在是复习的好时机。[[12-3-Lvalue-references|12.3 - 左值引用]]、[[12-4-Lvalue-references-to-const|12.4 - const类型的左值引用]]和[[12-5-Pass-by-lvalue-reference|12.5 - 传递左值引用]]都是非常有帮助的材料。
...
在上面的例子中，我们使用取地址运算符来获取变量x的地址并将其打印处理。内存地址通常是以十六进制值的形式打印的（参考[[5-3-Numeral-systems-decimal-binary-hexadecimal-and-octal|5.3 - 数值系统（十进制、二进制、十六进制和八进制）]]了解十六进制的内容），但通常不包括前缀`0x`。
...
> [!tldr] "关键信息"
> 在使用指针时，如果不同时使用[[dereference-operator|解引用运算符]]的时候，我们使用的是指针存放的地址。修改该地址就可以改变指针的指向 (`ptr = &y`)。
