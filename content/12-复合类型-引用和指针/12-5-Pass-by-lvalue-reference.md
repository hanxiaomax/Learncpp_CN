...
在前面的课程中，我们介绍了左值引用 ([[12-3-Lvalue-references|12.3 - 左值引用]]) 和指向const的左值引用 ([[12-4-Lvalue-references-to-const|12.4 - const类型的左值引用]])。单独来看，这两种引用看上去都没啥用——为什么我们能够直接访问变量却还要为其创建一个别名呢？
