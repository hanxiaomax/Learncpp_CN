...
在上节课中 ([[12-7-Introduction-to-pointers|12.7 - 指针简介]])，我们介绍了指针的基本知识——指针是一个对象，它持有另外一个对象的地址。我们可以使用[[dereference-operator|解引用运算符]]对地址进行解引用以获得该地址存放的值：
...
现代C++中应该避免使用上述两种字面量（应该使用`nullpt`来代替），我们会在[[12-11-Pass-by-address-part-2|12.11 - 按地址传递 Part2]]中介绍为什么要这么做。
