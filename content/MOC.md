## 1. C++ 基础

- [[1-1-Statements-and-the-structure-of-a-program|1.1 - 程序结构和语句]]
- [[1-2-comments|1.2 - 注释]]
- [[1-3-Introduction-to-objects-and-variables|1.3 - 对象和变量]]
- [[1-4-Variable-assignment-and-initialization|1.4 - 变量赋值和初始化]]
- [[1-5-Introduction-to-iostream-cout-cin-and-endl|1.5 - iostream 简介：cout，cin 和 endl]]
- [[1-6-Uninitialized-variables-and-undefined-behavior|1.6 - 未初始化变量和未定义行为]]
- [[1-7-Keywords-and-naming-identifiers|1.7 - 关键字和标识符]]
- [[1-8-Whitespace-and-basic-formatting|1.8 - 空白符和基本排版]]
- [[1-9-Introduction-to-literals-and-operators|1.9 - 字面量和操作符]]
- [[1-10-Introduction-to-expressions|1.10 - 表达式简介]]
- [[1-11-Developing-your-first-program|1.11 - 开发你的第一个程序]]
- [[1-x-Chapter-1-summary-and-quiz|1.x - 小结与测试 - C++基础]]

## 2. C++ 基础——函数和文件 

- [[2-1-Introduction-to-functions|2.1 - 函数简介]]
- [[2-2-Function-return-values-value-returning functions|2.2 - 函数返回值]]
- [[2-3-Void-functions-non-value returning functions|2.3 - void 函数]]
- [[2-4-Introduction-to-function-parameters-and-arguments|2.4 - 函数形参和实参]]
- [[2-5-Introduction-to-local-scope|2.5 - 局部作用域]]
- [[2-6-Why-functions-are-useful-and-how-to-use-them-effectively|2.6 - 为什么函数有用，如何高效使用函数]]
- [[2-7-Forward-declarations-and-definitions|2.7 - 前向声明和定义]]
- [[2-8-Programs-with-multiple-code-files|2.8 - 多文件程序]]
- [[2-9-Naming-collisions-and-an-introduction-to-namespaces|2.9 - 命名冲突和命名空间]]
- [[2-10-Introduction-to-the-preprocessor|2.10 - 预处理器简介]]
- [[2-11-Header-files|2.11 - 头文件]]
- [[2-12-Header-guards|2.12 - 头文件防卫式声明]]
- [[2-13-How-to-design-your-first-programs|2.13 - 如何设计第一个程序]]
- [[2-x-Chapter-2-summary-and-quiz|2.x - 小结与测试 - 函数与文件]]

## 3. 调试 C++ 程序 🚧

- [[3-1-Syntax-and-semantic-errors|3.1 - 语法和语义错误]]
- [[3-2-The-debugging-process|3.2 - 代码调试步骤]]
- [[3-3-A-strategy-for-debugging|3.3 - 代码调试策略]]
- [[3-4-Basic-debugging-tactics|3.4 — 基本代码调试技术]]
- [[3-5-More-debugging-tactics|3.5 - 更多代码调试技术]]
- [ ] [[3-6-Using-an-integrated-debugger-Stepping|3.6 - 使用集成调试器之单步调试]]
- [ ] [[3-7-Using-an-integrated-debugger-Running-and-breakpoints|3.7 - 使用集成调试器之运行和断点]]
- [ ] [[3-8-Using-an-integrated-debugger-Watching-variables|3.8 - 使用集成调试器之监控变量]]
- [ ] [[3-9-Using-an-integrated-debugger-The-call-stack|3.9 - 使用集成调试器之调用栈]]
- [ ] [[3-10-Finding-issues-before-they-become-problems|3.10 - 在问题发生前发现问题]]
- [ ] [[3-x-Chapter-3-summary-and-quiz|3.x - 小结与测试 - 代码调试]]

## 4. 基础数据类型 

- [[4-1-Introduction-to-fundamental-data-types|4.1 - 基础数据类型简介]]
- [[4-2-Void|4.2 - Void]]
- [[4-3-Object-sizes-and-the-sizeof-operator|4.3 - 对象的大小和 sizeof 操作符]]
- [[4-4-Signed-integers|4.4 - 有符号整型]]
- [[4-5-Unsigned-integers-and-why-to-avoid-them|4.5 - 无符号整型以及为什么要避免使用它]]
- [[4-6-Fixed-width-integers-and-size_t|4.6 - 固定宽度整型和 size_t]]
- [[4-7-Introduction-to-scientific-notation|4.7 - 科学计数法]]
- [[4-8-Floating-point-numbers|4.8 - 浮点数]]
- [[4-9-Boolean-values|4.9 - 布尔值]]
- [[4-10-Introduction-to-if-statements|4.10 - if 语句简介]]
- [[4-11-Chars|4.11 - 字符]]
- [[4-12-Introduction-to-type-conversion-and-static_cast|4.12 - 类型转换和 static_cast]]
- [[5-1-Const-variables-and-symbolic-constants|4.13 - const 变量和符号常量]]
- [[4-14-Compile-time-constants-constant-expressions-and-constexpr|4.14 - 编译时常量、常量表达式和 constexpr]]
- [[5-2-Literals|4.15 - 字面量]]
- [[5-3-Numeral-systems-decimal-binary-hexadecimal-and-octal|4.16 - 数值系统（十进制、二进制、十六进制和八进制）]]
- [[5-7-an-introduction-to-std-string|4.17 - std::string 简介]]
- [[5-8-Introduction-to-std-string_view|4.18 - std::string_view 简介]]
- [[4-x-Chapter-4-summary-and-quiz|4.x - 小结与测试 - 基本数据类型]]

## 5. 运算符 

- [[6-1-Operator-precedence-and-associativity|5.1 - 运算符优先级和结合律]]
- [[6-2-Arithmetic-operators|5.2 - 数学运算符]]
- [[6-3-Modulus-and-Exponentiation|5.3 - 求模和指数运算]]
- [[6-4-Increment-decrement-operators-and-side-effects|5.4 - 自增自减运算符及其副作用]]
- [[6-5-Comma-and-conditional-operators|5.5 - 逗号和条件运算符]]
- [[6-7-Relational-operators-and-floating-point-comparisons|5.6 - 关系运算符和浮点数比较]]
- [[6-8-Logical-operators|5.7 - 逻辑运算符]]
- [[06-运算符/6-x-Chapter-6-summary-and-quiz|5.x - 小结与测试 - 运算符]]

## 6. 作用域、生命周期和链接 

- [[7-1-Compound-statements-blocks|6.1 - 复合语句（语句块）]]
- [[7-2-User-defined-namespaces-and-the-scope-resolution-operator|6.2 - 用户定义命名空间和作用域解析运算符]]
- [[7-3-Local-variables|6.3 - 局部变量]]
- [[7-4-Introduction-to-global-variables|6.4 - 全局变量]]
- [[7-5-Variable-shadowing-name-hiding|6.5 - 变量遮蔽]]
- [[7-6-Internal-linkage|6.6 - 内部链接]]
- [[7-7-External-linkage-and-variable-forward-declarations|6.7 - 外部链接和变量前向声明]]
- [[7-8-Why-non-const-global-variables-are-evil|6.8 - 为什么非 const 全局变量是魔鬼]]
- [[7-10-Sharing-global-constants-across-multiple-files-using-inline-variables|6.9 - 使用 inline 变量共享全局常量]]
- [[7-11-Static-local-variables|6.10 - 静态局部变量]]
- [[7-12-Scope-duration-and-linkage-summary|6.11 - 作用域和链接小结]]
- [[7-13-Using-declarations-and-using directives|6.12 - using 声明和 using 指令]]
- [[7-9-Inline-functions|6.13 - 内联函数]]
- [[F-1-Constexpr-and-consteval-functions|6.14 - Constexpr 和 consteval 函数]]
- [[7-14-Unnamed-and-inline-namespaces|6.15 - 未命名和内联命名空间]]
- [[7-x-Chapter-7-summary-and-quiz|6.x - 小结与测试 - 作用域、持续时间和链接]]

## 7. 控制流和错误处理  🚧

- [[8-1-Control-flow-introduction|7.1 - 控制流]]
- [[8-2-If-statements-and-blocks|7.2 - if 语句和语句块]]
- [[8-3-Common-if-statement-problems|7.3 - 常见的 if 语句错误]]
- [[8-5-Switch-statement-basics|7.4 - switch 语句基础]]
- [[8-6-Switch-fallthrough-and-scoping|7.5 - switch fallthrough属性和作用域]]
- [[8-7-Goto-statements|7.6 - goto 语句]]
- [[8-8-Intro-to-loops-and-while-statements|7.7 - 循环和 while 语句]]
- [[8-9-Do-while-statements|7.8 - do-while 语句]]
- [[8-10-For-statements|7.9 - for 语句]]
- [[8-11-Break-and-continue|7.10 - break 和 continue]]
- [[8-12-Halts-exiting-your-program-early|7.11 - 挂起（提前结束程序）]]
- [[9-1-Introduction-to-testing-your-code|7.12 - 代码测试]]
- [[9-2-Code-coverage|7.13 - 代码覆盖率]]
- [[9-3-Common-semantic-errors-in-c|7.14 - C++ 中常见的语义错误]]
- [ ] [[9-4-Detecting-and-handling-errors|7.15 - 发现错误和处理错误]]
- [ ] [[9-5-std-cin-and-handling-invalid-input|7.16 - std::in 和输入错误处理]]
- [ ] [[9-6-assert-and-static-assert|7.17 - 断言和 static_assert]]
- [ ] [[8-13-introduction-to-random-number-generation|7.18 - 随机数生成器简介]]
- [ ] [[8-14-generating-random-numbers-using-mersenne-twister|7.19 - 使用 Mersenne Twister 生成随机数]]
- [ ] [[08-控制流/8-x-Chapter-8-summary-and-quiz|7.x - 小结与测试 - 控制流和错误处理]]


## 8. 类型转换和函数重载 

- [[10-1-Implicit-type-conversion-coercion|8.1 - 隐式类型转换]]
- [[10-2-Floating-point-and-integral-promotion|8.2 - 浮点数和整型提升]]
- [[10-3-Numeric-conversions|8.3 - 数值转换]]
- [[10-5-Arithmetic-conversions|8.4 - 算术转换]]
- [[10-6-Explicit-type-conversion-casting-and-static-cast|8.5 - 显式类型转换和static_cast]]
- [[10-7-Typedefs-and-type-aliases|8.6 - typedef 和类型别名]]
- [[10-8-Type-deduction-for-objects-using-the auto-keyword|8.7 - 使用 auto 关键字进行类型推断]]
- [[10-9-Type-deduction-for-functions|8.8 - 函数的类型推断]]
- [[11-1-Introduction-to-function-overloading|8.9 - 函数重载]]
- [[11-2-Function-overload-differentiation|8.10 - 重载函数的区分]]
- [[11-3-Function-overload-resolution-and-ambiguous-matches|8.11 - 函数重载解析和不明确匹配]]
- [[11-5-Default-arguments|8.12 - 默认参数]]
- [[11-6-Function-templates|8.13 - 函数模板]]
- [[11-7-Function-template-instantiation|8.14 - 函数模板的实例化]]
- [[11-8-Function-templates-with-multiple-template-types|8.15 - 具有多种类型的函数模板]]
- [[10-x-Chapter-10-summary-and-quiz|8.x - 小结与测试]]

## 9. 复合类型-引用和指针 

- [[12-1-Introduction-to-compound-data-types|9.1 - 复合数据类型]]
- [[12-2-Value-categories-lvalues-and-rvalues|9.2 - 值的分类（左值和右值）]]
- [[12-3-Lvalue-references|9.3 - 左值引用]]
- [[12-4-Lvalue-references-to-const|9.4 - const类型的左值引用]]
- [[12-5-Pass-by-lvalue-reference|9.5 - 传递左值引用]]
- [[12-7-Introduction-to-pointers|9.6 - 指针简介]]
- [[12-8-Null-pointers|9.7 - 空指针]]
- [[12-9-Pointers-and-const|9.8 - 指针和const]]
- [[12-10-Pass-by-address|9.9 - 按地址传递]]
- [[12-11-Pass-by-address-part-2|9.10 - 按地址传递 Part2]]
- [[12-12-Return-by-reference-and-return-by-address|9.11 - 按引用返回和按地址返回]]
- [[12-14-Type-deduction-with-pointers-references-and-const|9.12 - 指针、引用和const的类型推断]]
- [[12-复合类型-引用和指针/12-x-Chapter-12-comprehensive-quiz|9.x - 小结与测试 - 引用和指针]]


## 10. 复合类型-枚举和结构体 

- [[13-1-Introduction-to-program-defined-user-defined-types|10.1 - 程序定义类型简介]]
- [[13-2-unscoped-enumerations|10.2 - 无作用域枚举类型]]
- [[13-3-unscoped-enumeration-input-and-output|10.3 - 限定作用域枚举类型的输入输出]]
- [[13-6-scoped-enumerations-enum-classes|10.4 - 限定作用域枚举（枚举类）]]
- [[13-7-Introduction-to-structs-members-and-member-selection|10.5 - 结构体、成员和成员选择]]
- [[13-8-struct-aggregate-initialization|10.6 - 结构体的聚合初始化]]
- [[13-9-default-member-initialization|10.7 - 默认成员初始化]]
- [[13-10-struct-passing-and-miscellany|10.8 - 结构体传递及其他]]
- [[13-12-member-selection-with-pointers-and-references|10.9 - 基于指针和引用的成员选择]]
- [[13-13-class-templates|10.10 - 类模板]]
- [[13-14-class-template-argument-deduction-and-deduction -guides|10.11 - 类模板参数推断CTAD]] 
- [[13-复合类型-枚举和结构体/13-x-Chapter-13-comprehensive-quiz|10.x - 小结与测试 - 枚举和结构体]]
- [[13-y-using-a-language-reference|10.y - 学会使用C++参考手册]]

## 11. 数组，字符串和动态内存分配 

- [[11-1-Arrays-Part-I|11.1 - 数组（第一部分）]]
- [[11-2-Arrays-Part-II|11.2 - 数组（第二部分）]]
- [[16-6-Arrays-and-loops|11.3 - 数组和循环]]
- [[18-1-Sorting-an-array-using-selection-sort|11.4 - 数组排序之选择排序]]
- [[17-12-Multidimensional-Arrays|11.5 - 多维数组]]
- [[17-10-C-style-strings|11.6 - C 语言风格字符串]]
- [[5-9-std-string-view-part-2|11.7 - std::string_view（第二部分）]]
- [[17-8-Pointers-and-arrays|11.8 - 指针和数组]]
- [[17-9-Pointer-arithmetic-and-array-indexing|11.9 - 指针运算和数组索引]]
- [[17-11-C-style-string-symbolic-constants|11.10 - C 风格字符串符号常量]]
- [[19-1-Dynamic-memory-allocation-with-new-and-delete|11.11 - 使用 new 和 delete 进行动态内存分配]]
- [[19-2-Dynamically-allocating-arrays|11.12 - 动态分配数组]]
- [[16-8-For-each-loops|11.13 - for-each 循环]]
- [[19-5-Void-pointers|11.14 - void 指针]]
- [ ] [[19-4-Pointers-to-pointers-and-dynamic-multidimensional-arrays|11.15 - 指向指针的指针和多维数组]]
- [[17-1-An-introduction-to-std-array|11.16 - 固定数组 std::array 简介]]
- [[16-2-An-introduction-to-std-vector|11.17 - 动态数组 std::vector 简介]]
- [[18-2-Introduction-to-iterators|11.18 - 迭代器简介]]
- [[18-3-Introduction-to-standard-library-algorithms|11.19 - 标准库算法简介]]
- [[17-数组,字符串和动态内存分配/17-x-chapter-17-comprehensive-quiz|11.x - 小结与测试 - 数组、字符串和动态分配]]

## 12. 函数 

- [[20-1-function-pointers|12.1 - 函数指针]]
- [[20-2-the-stack-and-the-heap|12.2 - 栈和堆]]
- [[16-10-std-vector-capacity-and-stack-behavior|12.3 - std::vector的容量和类栈行为]]
- [[20-3-recursion|12.4 - 递归]]
- [[20-4-command-line-arguments|12.5 - 命令行参数]]
- [ ] [[20-5-Ellipsis-and-why-to-avoid-them|12.6 - 省略号以及为什么要避免使用它]]
- [[20-6-introduction-to-lambdas-anonymous-functions|12.7 - lambda表达式简介]]
- [[20-7-lambda-captures|12.8 - lambda 闭包]]
- [[20-函数/20-x-Chapter-20-comprehensive-quiz|12.x - 小结与测试 - 函数]]

## 13. 面向对象编程基础 

- [[14-1-welcome-to-object-oriented-programming|13.1 - 欢迎来到面向对象的世界]]
- [[14-2-classes-and-class-members|13.2 - 类和类成员]]
- [[14-5-public-vs-private-access-specifiers|13.3 - 访问修饰符 public 和 private]]
- [[14-6-access-functions-and-encapsulation|13.4 - 成员访问函数与封装]]
- [[14-9-constructors|13.5 - 构造函数]]
- [[14-10-constructor-member-initializer-lists|13.6 - 构造函数成员初始化值列表]]
- [[13-7-non-static-member-initialization|13.7 - 非静态成员初始化]]
- [[14-12-overlapping-and-delegating-constructors|13.8 - 重叠和委派构造函数]]
- [[19-3-destructors|13.9 - 析构函数]]
- [[15-1-the-hidden-this-pointer|13.10 - 隐藏的this指针]]
- [[15-2-class-code-and-header-files|13.11 - 类代码和头文件]]
- [[14-4-const-class-objects-and-member-functions|13.12 - const 对象和成员函数]]
- [[15-6-static-member-variables|13.13 - 静态成员变量]]
- [[15-7-static-member-functions|13.14 - 静态成员函数]]
- [[15-8-friend-functions-and-classes|13.15 - 友元函数和友元类]]
- [[14-13-anonymous-objects|13.16 - 匿名对象]]
- [[15-3-nested-types-in-classes|13.17 - 类中的嵌套类型]]
- [[18-4-timing-your-code|13.18 - 对程序进行计时]]
- [[14-面向对象编程基础/14-x-Chapter-14-comprehensive-quiz|13.x - 小结与测试 - 面向对象基础]]

## 14. 操作符重载  🚧

- [[21-1-introduction-to-operator-overloading|14.1 - 运算符重载简介]]
- [ ] [[21-2-overloading-the-arithmetic-operators-using-friend-functions|14.2 - 使用友元重载算数运算符]]
- [ ] [[21-3-overloading-operators-using-normal-functions|14.3 - 使用普通函数重载运算符]]
- [ ] [[21-4-overloading-the-IO-operators|14.4 - 重载输入输出运算符]]
- [ ] [[21-5-overloading-operators-using-member-functions|14.5 - 使用成员函数重载运算符]]
- [ ] [[21-6-overloading-unary-operators-+--and-!|14.6 - 重载一元运算符+，- 和 !]]
- [ ] [[21-7-overloading-the-comparison-operators|14.7 - 重载比较运算符]]
- [ ] [[21-8-overloading-the-increment-and-decrement-operators|14.8 - 重载递增递减运算符]]
- [ ] [[21-9-overloading-the-subscript-operator|14.9 - 重载下标运算符]]
- [ ] [[21-10-overloading-the-parenthesis-operator|14.10 - 重载括号运算符]]
- [ ] [[21-11-Overloading-typecasts|14.11 - 重载类型转换操作符]]
- [[21-x-Chapter-21-comprehensive-quiz|14.x - 小结与测试 - 运算符重载]]

## 16. 对象关系简介 

- [[23-1-object-relationships|16.1 - 对象关系]]
- [[23-2-composition|16.2 - 组合关系]]
- [[23-3-aggregation|16.3 - 聚合关系]]
- [[23-4-association|16.4 - 关联关系]]
- [[23-5-dependencies|16.5 - 依赖关系]]
- [[23-6-container-classes|16.6 - 容器类]]
- [[23-7-std-initializer_list|16.7 - std::initializer_list]]
- [[23-x-Chapter-23-comprehensive-quiz|16.x - 小结与测试 - 对象关系]]

## 17. 继承 

- [[24-1-introduction-to-inheritance|17.1 - 继承简介]]
- [[24-2-basic-inheritance-in-c++|17.2 - C++继承基础]]
- [[24-3-order-of-construction-of-derived-classes|17.3 - 派生类的构造顺序]]
- [[24-4-constructors-and-initialization-of-derived-classes|17.4 - 派生类的构造和初始化]]
- [[24-5-inheritance-and-access-specifiers|17.5 - 继承和访问说明符]]
- [[24-6-adding-new-functionality-to-a-derived-class|17.6 - 为派生类添加新功能]]
- [[24-7-calling-inherited-functions-and-overriding-behavior|17.7 - 调用继承函数和重写]]
- [[24-8-Hiding-inherited-functionality|17.8 - 隐藏继承的函数]]
- [[24-9-multiple-inheritance|17.9 - 多重继承]]
- [[24-x-Chapter-24-comprehensive-quiz|17.x - 小结与测试 - 继承]]

## 18. 虚函数 

- [[25-1-pointers-and-references-to-the-base-class-of-derived-objects|18.1 - 基类的指针和引用]]
- [[25-2-virtual-functions-and-polymorphism|18.2 - 虚函数和多态]]
- [[25-3-the-override-and-final-specifiers-and-covariant-return-types|18.3 - override、final标识符以及协变返回类型]]
- [[25-4-virtual-destructors-virtual-assignment-and-overriding-virtualization|18.4 - 虚构造函数、虚赋值和重载虚拟化]]
- [[25-5-early-binding-and-late-binding|18.5 - 早期绑定和后期绑定]]
- [[25-6-the-virtual-table|18.6 - 虚函数表]]
- [[25-7-pure-virtual-functions-abstract-base-classes-and- interface-classes|18.7 - 纯虚函数，抽象基类和接类]]
- [[25-8-virtual-base-classes|18.8 - 虚基类]]
- [[25-9-object-slicing|18.9 - 对象切片]]
- [[25-10-dynamic-casting|18.10 - 动态类型转换]]
- [[25-11-printing-inherited-classes-using-insertion-operator|18.11 - 使用<<运算符打印继承类]]
- [[25-x-Chapter-25-comprehensive-quiz|18.x - 小结与测试 - 虚函数]]

## 19. 模板和类  🚧
- [ ] [[26-x-Chapter-26-comprehensive-quiz|19.x - 小结与测试 - 模板和类]]

## 20. 异常  🚧
- [ ] [[27-x-Chapter-27-comprehensive-quiz|20.x - 小结与测试 - 异常]]
- [ ] [[20.4]]

## M. move 和智能指针 

- [[22-1-introduction-to-smart-pointers-and-move-semantics|M.1 - 智能指针和移动语义简介]]
- [[22-2-R-value-references|M.2 - 右值引用]]
- [[22-3-move-constructors-and-move-assignment|M.3 - 移动构造函数和移动赋值]]
- [[22-4-std-move|M.4 - std::move]]
- [ ] [[27-10-std-move-if-noexcept|M.5 - std::move_if_noexcept]]
- [[22-5-std-unique-ptr|M.6 — std::unique_ptr]]
- [[22-6-std-shared-ptr|M.7 — std::shared_ptr]]
- [ ] [[22-7-circular-dependency-issues-with-std-shared-ptr-and-std-weak-ptr|M.8 — 智能指针带来的循环依赖问题]]
-  [[22-x-chapter-M-comprehensive-review|M.x - 小结与测试 - 移动和智能指针]]

## 21. STL 

- [[21-1-the-standard-library|21.1 - 标准库]]
- [[21-2-STL-containers-overview|21.2 - STL容器]]
- [[21-3-STL-iterators-overview|21.3 - STL迭代器]]
- [[21-4-STL-algorithms-overview|21.4 - STL算法]]

## 22. std 字符串 

- [[22-1-std-string-and-std-wstring|22.1 - std::string 和 std::wstring]]
- [[22-2-std-string-construction-and-destruction|22.2 - std-string 的构造和析构]]
- [[22-3-std-string-length-and-capacity|22.3 - std-string 的长度和容量]]
- [[22-4-std-string-character-access-and-conversion-to-C-style-arrays|22.4 - std-string 字符访问和C字符数组转换]]
- [[22-5-std-string-assignment-and-swapping|22.5 - std-string 赋值和交换]]
- [[22-6-std-string-appending|22.6 - std-string 追加]]
- [[22-7-std-string-inserting|22.7 - std-string 插入]]


## 23. 输入输出 

- [[28-1-Input-and-output-IO-streams|23.1 - 输入输出流]]
- [[28-2-Input-with-istream|23.2 - 使用istream处理输入]]
- [[28-3-output-with-ostream-and-ios|23.3 - 使用ostream和ios处理输出]]
- [[28-4-stream-classes-for-strings|23.4 - 用于字符串的 stream 类]]
- [[28-5-stream-states-and-input-validation|23.5 - 流状态和输入验证]]
- [[28-6-basic-file-IO|23.6 - 基本文件输入输出]]
- [[28-7-random-file-IO|23.7 - 随机文件输入输出]]


## A. 其他话题  

- [[A-1-static-and-dynamic-libraries|A.1 - 静态库和动态库]]
- [[A-2-using-libraries-with-visual-studio|A.2 - 在visual studio中使用库]]
- [[A-3-using-libraries-with-Code-Blocks|A.3 - 在Code Blocks中使用库]]
- [[A-4-C++FAQ|A.4 — C++ FAQ]]

## B. C++标准更新 

- [[B-1-introduction-to-C++11|B.1 - C++11]]
- [[B-2-introduction-to-C++14|B.2 - C++14]]
- [[B-3-introduction-to-C++17|B.3 - C++17]]
- [[B-4-introduction-to-C++20|B.4 - C++20]]

## O1. 位运算  🚧

- [ ] [[O-1-bit-flags-and-bit-manipulation-via-std-bitset|O.1 - bit标记和通过std::bitset操作bit]]
- [ ] [[O-2-bitwise-operators|O.2 - 位运算符]]
- [ ] [[O-3-bit-manipulation-with-bitwise-operators-and-bit-masks|O.3 - 使用位运算符和掩码操作位]]
- [ ] [[O-4-converting-between-binary-and-decimal|O.4 - 二进制和十进制转换]]

