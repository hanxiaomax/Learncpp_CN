...
>[!INFO] 扩展阅读
> C++ 的输入输出库并没有提供一种可以避免按下回车的方式来获取输入结果。如果你希望实现该功能，你必须使用第三方库来完成。如果是开发控制台应用程序，我们推荐尝试 [pdcurses](https://pdcurses.org/) 这个库。另外，很多图形用户库界面库也提供了类似的功能。
...
-   `<<` 配合`std::cout` 使用，并指明了数据的流动方向 (假设 `std::cout` 表示的是控制台 the console, the output data is moving from the variable to the console). `std::cout << 4` moves the value of 4 to the console
